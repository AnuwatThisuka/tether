...
- **Uncommitted changes**: generate a commit message from the staged
  ...
  staged, `git add -A`. Then `git commit` with a HEREDOC message. Keep the
  message short and specific — do not write a PR-body-style
  description here. Prefer describing the failure mode or correctness
  win, not only the file list. End the message with the trailer:
...
- **Title**: short, imperative, derived from the most recent commit
  message or the scope of the diff (e.g. `fix(wal): persist before ack`).
- **Body**: concise. Summary + a short Test Plan checklist. If the
  change touches WAL/offsets/snapshot handoff, say whether
  `make test-integration` was run (or honestly that it was not). Skip
  sections that have nothing meaningful to say — do not pad. End the
  body with the trailer:
...
- Do not run a full `AGENTS.md` standards review in this skill. The
  button is a fast path; use `/create-pr` (the general-purpose command)
  for the gated review flow.
- **Invariants floor**: even on the fast path, if the diff obviously
  breaks one of the seven Invariants in `AGENTS.md` (ack before durable
  persist, client-supplied shape predicate, non-idempotent mutations,
  snapshot/stream overlap or gap, continue after schema drift, unbounded
  replication slot hold, or blocking the WAL reader on a slow client) —
  STOP, do not open the PR, and surface it to the user. Never work
  around an Invariant silently.
